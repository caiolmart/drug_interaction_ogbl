...
class DeepwalkTrainer:
    ...
    def init_device_emb(self):
        ...
        if self.only_gpu:
            ...
            print("Mix CPU with %d GPU" % len(self.gpus))
            if len(self.gpus) == 1:
                assert self.gpus[0] >= 0, 'mix CPU with GPU should have available GPU'
                self.emb_model.set_device(self.gpus[0])
        else:
            print("Run in CPU process")
            self.gpus = [torch.device('cpu')]
    ...
    def fast_train_mp(self):
        ...
        for i in range(len(self.gpus)):
            p = mp.Process(target=self.fast_train_sp, args=(i, self.gpus[i]))
            ps.append(p)
            p.start()
    ...
    def fast_train_sp(self, rank, gpu_id):
        ...
        num_pos = int(2 * self.walk_length * self.window_size\
                ...
                if i > 0 and i % self.print_interval == 0:
                    if self.print_loss:
                        print("GPU-[%d] batch %d time: %.2fs loss: %.4f" \
                            % (gpu_id, i, time.time()-start, -sum(self.emb_model.loss)/self.print_interval))
                        self.emb_model.loss = []
                    else:
    ...
    def fast_train(self):
        ...
        num_pos = 2 * self.walk_length * self.window_size\
            - self.window_size * (self.window_size + 1)
        num_pos = int(num_pos)
                ...
                if i > 0 and i % self.print_interval == 0:
                    if self.print_loss:
                        print("Batch %d training time: %.2fs loss: %.4f" \
                            % (i, time.time()-start, -sum(self.emb_model.loss)/self.print_interval))
                        self.emb_model.loss = []
                    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_file', type=str, 
            help="path of the txt network file, builtin graph include youtube-net and blog-net") 
    ## ogbl graphs
    parser.add_argument('--ogbl_name', type=str, 
            help="name of ogbl graph, e.g. ogbl-ddi")
    parser.add_argument('--load_from_ogbl', default=False, action="store_true",
            help="whether load graph from ogbl")
    ...
    if async_update:
        assert mix, "--async_update only with --mix"
